...
参考地址：https://cloud.tencent.com/developer/article/1377103

我们这里只实现了简单的计数器方案，具体可查看代码提交 commit：**47e78ac**

同时使用了拦截器来实现通用的接口限流功能，具体看代码提交 commit: **c7f7060**

##### 计数器方案
计数器方案顾名思义，就是对请求进行计数，当达到计数上限后，后续的请求就直接返回失败接口。比如现在要对一个接口限流为 QPS = 100，那么在 1s 内，来一个请求计数就加1，等到了100以后，后续的请求直接返回，等过了1s后，计数归零，再重新开始计数。

在实际实现的时候，我们可以通过 **AtomicLong#incrementAndGet()** 方法来给计数器加1并返回最新值，通过这个最新值和阈值进行比较。

计数器的方案比较简单，但缺点也很明显，就是假如 1s 内的前 10ms 就来了 100 个请求后，那么后面的 990ms 内的所有请求都会被拒绝掉，服务器资源产生了大量的浪费，这种现象也叫 “突刺现象”。

##### 漏桶方案
漏桶方案可以解决计数器方案带来的“突刺现象”。它就像一个漏斗一样，不管进来的请求速率有多少，它都以横定的速率流出。比如要实现 QPS = 100 的限流，那么它会每隔 10ms 处理一个请求。

在实际实现的时候，我们可以通过一个队列来保存过来的请求，然后有一个线程每隔 10ms 来请求队列中的消息。

但漏斗也会有容量的限制，这也就导致了一旦超过了限制后，后续的请求就会丢失掉，即它也会存在弊端：无法应对短时间的突发流量。

##### 令牌桶方案
令牌桶方案可以一定程度的解决上述漏桶方案导致的短时间内无法处理大量流量的弊端。它有一个固定容量的桶来专门存放令牌，同时还有一个线程来以一定的速率往桶中存放令牌，桶中令牌的数量有上限，多余的会直接丢弃。当请求过来的时候，会从桶中获取令牌，如果获取到令牌，那么就继续后续正常的业务，如果桶中没有令牌，那么请求就被丢弃掉。

通过这种方案，在服务一开始的时候，令牌桶中有大量的令牌，那么每个过来的请求都可以获取到令牌，当令牌桶中没有令牌后，那么就会以一定的速率补充令牌，所以后续的请求就以一定的速率通过，这样就实现了一开始处理突发的大量流量，后续又能以固定速度处理请求

在具体实现的时候，用一个队列来保存令牌，同时通过另外一个线程池来固定的速率往队列中放令牌，每来一个请求就从队列中尝试获取一个令牌，得到令牌后就继续执行。
